...
def plot_weather_heatmap(df: pd.DataFrame, outdir: str) -> str:
    ...
    heatmap_data = monthly_temp.pivot(
        index="city",
        columns="month",
        values="avg_temp"
    )
...
def plot_weather_scatter(df: pd.DataFrame, outdir: str) -> str:
    ...
    if max_precip > 0:
        precip_levels = np.linspace(0, max_precip, 4)
    else:
        precip_levels = [0]
    ...
    for p in precip_levels:
        marker_size = np.interp(
            p,
            [0, max_precip if max_precip > 0 else 1],
            size_range
        )
...
def plot_minnesota_precip_line(df: pd.DataFrame, outdir: str) -> str:
    ...
    # Remove invalid rows
    df = df.dropna(subset=["year", "mo", "precip", "site"])
